...
int main( int argc, const char** argv ) {
    ...
    for(unsigned int j = 0; j < cpu_output.rows; j++){
        for(unsigned int i = 0; i < cpu_output.cols; i++){
            unsigned char cpu_pixel = cpu_output.data[cpu_output.cols * j + i ];
            unsigned char gpu_pixel = gpu_output.data[gpu_output.cols * j + i ];
